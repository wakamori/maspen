...
class assign {
	...
	function view_assign_status($viewAll){
		global $CFG, $DB, $USER, $PAGE;

		$list = array();
		$list["Name"] = $this->get_instance()->name;
		$list["Intro"] = strip_tags($this->get_instance()->intro);

		$str = strip_tags($this->view_student_summary($USER, false));
		$o = explode("\n", $str);
		$i = 1; $key = null; $value = null;
		while($o[$i] != "Online text"){
			if(trim($o[$i]) != ""){
				if($key == null){
					$key = $o[$i];
				}
		...
		if($viewAll == 0){
			...
			$list["Online text"] = $value;
		}
		else {
			$submission = $this->get_user_submission($USER->id,false);
			$submissionid = $submission->id;
			$plugintype = "onlinetext";
			$item = null;
			...
			$o = $this->output->render(new assign_submission_plugin_submission(
					$plugin,
					$item,
					assign_submission_plugin_submission::FULL,
					$this->get_course_module()->id,
	 ...
	 * @return bool
	 */
	private function process_save_submission(&$mform) { //Jamp
		global $USER, $CFG;
	...
	function submit_assign($id, $text) { //Jamp
		global $USER, $CFG;
		...
		return $this->view_assign_status(0);
		//    	}
		//return false;
	}
